...
const Account = () => {
  ...
  useEffect(() => {
    ...
    // Subscribe to real-time additions/removals across open tabs
    const unsubscribe = onSnapshot(userRef, async (docSnap) => {
      if (docSnap.exists()) {
        const userData = docSnap.data();
        const movieIds = userData.favoriteMovies || [];
        const tvIds = userData.favoriteTvShows || [];
        const dbPlaylists = userData.playlists || []; // Pull real playlists directly from database document
        ...
        try {
          ...
          const resolvedTv = await Promise.all(tvPromises);
          ...
          // Map database structure cleanly into our application layer array state
          setPlaylists(dbPlaylists);
        } catch (error) {
          console.error("Error fetching TMDB detailed payloads for account dashboard:", error);
        } finally {
          setLoadingMedia(false);
        }
      } else {
        // Clear collections instantly on pristine user documents
        setFavoriteMovies([]);
        setFavoriteTvShows([]);
        setPlaylists([]);
        setLoadingMedia(false);
    ...
    <div className="min-h-screen bg-[#030014] text-white pt-28 pb-16 px-4 md:px-8 lg:px-12 max-w-[1600px] mx-auto">
      ...
      <div className="grid grid-cols-1 md:grid-cols-[260px_1fr] lg:grid-cols-[300px_1fr] gap-8 lg:gap-12 items-start">
                    ...
                    {(playlistsExpanded ? playlists : playlists.slice(0, 5)).map((playlist, idx) => (
                      <PlaylistCard 
                        ...
                        playlist={{
                          ...
                          cover_path: playlist.items?.[0]?.backdrop_path || null
                        }} 
                      />
                    ))}
                  </div>
        ...
        <div className="bg-[#151518] border border-white/5 rounded-2xl p-6 lg:p-8 shadow-2xl w-full">
            ...
            {/* Profile Pictures Action Row - Kept Centered */}
            ...
            {/* Fixed: Restored items-center to keep trashcan and label aligned vertically */}
            <div className="flex flex-col items-center gap-2 group cursor-pointer">
            ...
            {/* Change Username Input Section - Centered with max-w-2xl bounding box */}
            ...
            {/* Password Management Block - Entirely Centered via Column Wrapper */}
