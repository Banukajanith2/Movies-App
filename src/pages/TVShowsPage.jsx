...
const CATEGORIES = [
  ...
  { value: "on_the_air",  label: "On The Air",      icon: "📡" },
  { value: "upcoming",    label: "New & Upcoming",  icon: "📅" },
];
...
const buildDiscoverUrl = ({ category, page, selectedGenres, selectedProviders, language, rating, yearRange }) => {
  ...
  if (category === "on_the_air") {
    // air_date matches any episode air date, so this captures shows currently mid-run.
    const past = new Date(today);
    past.setDate(past.getDate() - 7);
    const future = new Date(today);
    future.setDate(future.getDate() + 7);
    url.searchParams.set("air_date.gte", isoDate(past));
    url.searchParams.set("air_date.lte", isoDate(future));
  } else if (category === "upcoming") {
    const tomorrow = new Date(today);
    tomorrow.setDate(tomorrow.getDate() + 1);
    url.searchParams.set("first_air_date.gte", isoDate(tomorrow));
  }
  ...
  if (yearObj.gte) {
    url.searchParams.set("first_air_date.gte", `${yearObj.gte}-01-01`);
    url.searchParams.set("first_air_date.lte", `${yearObj.lte}-12-31`);
  }
...
const TVShowsPage = () => {
  ...
  useEffect(() => {
    const fetchGenres = async () => {
      try {
        const res = await fetch(`${API_BASE_URL}/genre/tv/list?language=en-US`, API_OPTIONS);
  ...
  useEffect(() => {
    const fetchTvShows = async () => {
      ...
      try {
        ...
        if (!(data.results || []).length) setError("No TV shows match these filters.");
      } catch (err) {
          ...
          <div className="flex-1 min-w-0">
            ...
            <section className="search-results-grid">
              {loading ? (
                ...
                tvShows.map((tvShow) => (
                  <TvCard key={tvShow.id} tvShow={tvShow} className="tv-card" />
                ))
              )}
            </section>
